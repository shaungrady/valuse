...
Every mutation spreads the entire state object, and per-row render isolation
means writing selectors by hand.
...
and reactions. It pioneered fine-grained reactivity in React and its property
access is clean. The tradeoff is class-based models with `makeAutoObservable`,
`observer()` wrappers on every component, and generator-based `flow()` for async
with manual abort.
